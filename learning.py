...
def create_learning_dataset():
    ...
    for column in ['authors', 'directors', 'cast', 'genres', 'production_companies', 'production_countries']:
        ...
        encoded_column = df[column].apply(lambda x: pd.Series([1] * len(list(set(x.split(', ')))), index=list(set(x.split(', '))))).fillna(0, downcast='infer').astype(np.int0) # type: ignore
...
if __name__ == '__main__':
    ...
    print(f'Took {time.time() - t1} seconds to open CSV')
    print(f'Size in RAM is {sys.getsizeof(df)}')
